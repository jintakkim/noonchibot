...
class DerivativeUserStreamEventPublisher implements LifecycleComponent, WebsocketStatus {
    ...
    private String fetchListenKey() {
        RestRequest request = RestRequest.builder()
                ...
                .pathUrl(DerivativeApiSpec.LISTEN_KEY_PATH_URL)
                .authRequired(true)
                .build();
    ...
    private void connectionLoop() {
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                this.wsConnection = wsAssistant.connect(URI.create(DerivativeApiSpec.WSS_PRIVATE_URL + "/" + listenKey));
                while (true) {
    ...
    private void processMessage(String message) {
        ...
        if(eventType.equals("ORDER_TRADE_UPDATE")) {
            ...
            OrderUpdateEvent orderUpdate = new OrderUpdateEvent(
                    ...
                    DerivativeApiSpec.ORDER_STATE.get(orderMessage.get("X").asString()),
                    clientOrderId,
                    exchangeOrderId
            );
