...
class SpotOrderExecutor extends AbstractExchangeOrderExecutor {
    ...
    protected boolean isOrderNotFoundDuringCancellationException(Throwable e) {
        String message = e.getMessage();
        return message != null
                && message.contains(String.valueOf(SpotApiSpec.UNKNOWN_ORDER_DURING_CANCELLATION_ERROR_CODE));
    }
    ...
    protected OrderPlaceResult placeOrder(InFlightOrder inFlightOrder) {
        ...
        if (inFlightOrder.getOrderType() == OrderType.LIMIT) {
            ...
            if(!inFlightOrder.isPostOnly()) {
                apiParams.put("timeInForce", SpotApiSpec.TIME_IN_FORCE_API_VALUE.get(inFlightOrder.getTimeInForce()));
            }
        }
        RestRequest request = RestRequest.builder()
                .method(HttpMethod.POST)
                .pathUrl(SpotApiSpec.ORDER_PATH_URL)
                .authRequired(true)
                .params(apiParams)
                .build();
