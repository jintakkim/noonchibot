...
class DerivativeOrderExecutor extends AbstractExchangeOrderExecutor {
    ...
    protected boolean isOrderNotFoundDuringCancellationException(Throwable e) {
        String message = e.getMessage();
        return message != null
                && message.contains(String.valueOf(DerivativeApiSpec.UNKNOWN_ORDER_DURING_CANCELLATION_ERROR_CODE));
    };
    ...
    protected OrderPlacedDto placeOrder(InFlightOrder inFlightOrder) {
        ...
        if (inFlightOrder.getOrderType() == OrderType.LIMIT) {
            ...
            if(inFlightOrder.isPostOnly() && inFlightOrder.getTimeInForce() == TimeInForce.GTC) {
                ...
                apiParams.put("timeInForce", DerivativeApiSpec.TIME_IN_FORCE_API_VALUE.get(inFlightOrder.getTimeInForce()));
            }
        }
